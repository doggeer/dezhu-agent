# Spec：Hermes Agent 对话循环（第一版）

## 1. 背景与目标（Why）

语言模型本身只做"生成下一段内容"。如果没有一层代码在中间反复"调 API → 执行工具 → 写回结果 → 再调 API"，模型就只是一个"会说话的程序"，还不是一个"会干活的 agent"。

本项目（dezhu-agent）需要实现这个核心对话循环，作为整个 agent 系统的基础设施。第一版只实现**最小可行版本**，后续迭代逐步加入 Gateway、子 agent、异步工具桥接等复杂特性。

**成功指标**：给定一个带工具（如 `read_file`）的 agent，用户输入单条消息触发单次工具调用，循环能正确执行工具并返回最终回复。
